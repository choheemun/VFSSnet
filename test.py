...
def test():
    ...
    with tf.Session() as sess:
        ...
        if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
            ...
            saver.restore(sess, "dataset/checkpoint/checkpoint1/nn.ckpt-3303")              # 2.over fitting 이 확인되어 특정 epoch를 선택하여 학습을 이어가는 경우
        ...
        for step in range(10000):
            ...
            loss_val=sess.run(loss)  # feed_dict를 쓰지 않는다. input pipeline 이 data를 feed 해준다
            losses.append(loss_val)
            if step % 100 == 0:
                average_loss=sum(losses,0.0)/len(losses)
                print('Step: {:4d} | Average_loss: {:.5f}'.format(step, average_loss))
