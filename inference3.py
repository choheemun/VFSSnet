...
def inference():
    ...
    with graph_2.as_default():
        with tf.name_scope("second") as scope:
            ...
            x2 = tf.placeholder(tf.float32, shape=[1, 448, 448, 3])

            model = ResNet50(input_tensor=x2, include_top=False, weights='imagenet', pooling='max')
            model.trainable = False
    ...
    with tf.Session(graph=graph_2) as sess:
        ...
        for image_path in glob.glob("./dataset/photo/photoresize/*.png"):
            ...
            graph_val = sess.run(box, feed_dict={x2: img2})
...
'''
    ...
    with tf.Session(graph=graph_2) as sess:
        ...
        saver.restore(sess, "dataset/checkpoint/checkpoint2/nn.ckpt-301")
        ...
        for image_path in glob.glob("./dataset/photo/photoresize/*.png"):
            ...
            graph_point = sess.run(box, feed_dict={x: img})
